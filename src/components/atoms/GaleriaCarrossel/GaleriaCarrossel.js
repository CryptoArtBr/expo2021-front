...
const CarrosselItem = ({title, description, image, link}) => {
  return <div className={styles.carrossel_item}>
    <img src={image} alt="title" className={styles.carrossel_item__image}></img>
    <span className={`${styles.carrossel_item__info} ${styles.carrossel_item__bg}`}></span>
...
const CarrosselItems = ({items =[], offset}) => {
  return (
    <div className={styles.carrossel__images_container} style={{left: `-${offset}px`}}>
      {items.map((item, index) => <CarrosselItem key={index} {...item} />)}
    </div>
  )
}
  ...
  return <>
    <div ref={carrosselElement} className={styles.carrossel}>
      ...
      <div className={styles.carrossel__buttons_container}>
        <Button type="nav" onClick={() => navigate(-1)}>h</Button>
        <Button type="nav" onClick={() => navigate(1)}>e</Button>
      </div>
    </div>
    <div className={styles.item_indicators}>
      {items.map((item, index) => <span className={index === activeItem ? styles.active_item_indicator : styles.item_indicator}></span>)}
    </div>
  </>
}